...
const Shop = () => {
    ...
    const handleSearch = async e => {
        ...
        try {
            const response = await axios.post('http://localhost:5004/api/filter/search', { type: 'text', query: e.target.value });
    ...
    const handleCategory = async e => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:5004/api/filter/search', { type: 'category', query: updatedCategoryIds });
            dispatch({ type: GET_PRODUCTS, payload: response.data.products });
