...
function QuotationDetails() {
    ...
    const fetchData = async () => {
        try {
            ...
            if (response.data.success) {
                setShowQuotations(response.data.data.map(quotation => ({
                    ...quotation,
                    showAllImages: false
                })));
            }
    ...
    const toggleImages = (index) => {
        const updatedQuotations = [...showQuotations];
        updatedQuotations[index].showAllImages = !updatedQuotations[index].showAllImages;
        setShowQuotations(updatedQuotations);
    };
                            ...
                            <tr key={item._id}>
                                <td className="align-middle">
                                    {item.showAllImages ?
                                        ...
                                        <div>
                                            <img
                                                src={`http://localhost:4000/Images/${item.furniture_images[0]}`}
                                                alt=""
                                                className="img-fluid"
                                                style={{ maxHeight: "100px", maxWidth: "100px", margin: "5px" }}
                                            />
                                ...
                                <td className="align-middle">{item.company_description}</td>
