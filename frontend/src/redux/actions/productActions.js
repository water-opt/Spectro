...
export const createProduct = formData => async dispatch => {
    try{
        ...
        const response = await axios.post('http://localhost:5004/api/product',formData);
...
export const getProducts = () => async dispatch => {
    try{
        dispatch({ type: START_LOADING});
        const response = await axios.get('http://localhost:5004/api/product');
        dispatch({ type: STOP_LOADING});
...
export const getProduct = productId => async dispatch => {
    try{
        dispatch({ type: START_LOADING});
        const response = await axios.get(`http://localhost:5004/api/product/${productId}`);
        dispatch({ type: STOP_LOADING});
...
export const deleteProduct = productId => async dispatch => {
    try{
        dispatch({ type: START_LOADING});
        const response = await axios.delete(`http://localhost:5004/api/product/${productId}`);
        dispatch({ type: STOP_LOADING});
