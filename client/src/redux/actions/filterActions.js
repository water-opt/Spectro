...
export const getNewArrivals = (sortBy='desc', limit=4) => async dispatch => {
    try{
        ...
        const response = await axios.get(`http://localhost:5002/api/filter?sortBy=${sortBy}&limit=${limit}`);
...
export const getProductsByFilter = arg => async dispatch => {
    try{
        ...
        const response = await axios.get('http://localhost:5002/api/filter/search', arg);
